...
from pamap2.preprocessing import hpf
...
def get_dataset(train_persons, test_persons, frame_size, activities, attributes, positions, axes, preprocesses=None):
    if preprocesses is None: preprocesses = []

    pamap2 = PAMAP2('data_cache/PAMAP2_Dataset/Protocol/', cache_dir='data_cache/org/')
...
for model_name in model_list:
    ...
    for person in all_persons:
        ...
        for cut_off in [10, 20, 30, 40]:
            print('(Cutoff freq: {})'.format(cut_off))
            hpf_fn = lambda x: hpf(x, cut_off, 100)
            # ds = lambda x: downsampling(x, rate=(int(50/cut_off)))
            train_loader, test_loader = get_dataset(train_persons, test_persons, frame_size, activities, attributes, positions, axes, [hpf_fn])
            ...
            pd.DataFrame(hist).to_csv('{}_{}_hpf{}.csv'.format(model_name, person, cut_off))
