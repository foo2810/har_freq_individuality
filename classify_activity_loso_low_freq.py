...
for model_name in model_list:
    ...
    for person in all_persons:
        ...
        for cut_off in [10, 20, 30, 40, 50]:
            print('(Cutoff freq: {})'.format(cut_off))
            lpf_fn = lambda x: lpf(x, cut_off, 100)
            # ds = lambda x: downsampling(x, rate=(int(50/cut_off)))
            train_loader, test_loader = get_dataset(train_persons, test_persons, frame_size, activities, attributes, positions, axes, [lpf_fn])
